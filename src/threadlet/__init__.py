...
class Worker(threading.Thread):
    ...
    def submit(self, target: t.Callable, /, *args: t.Any, **kwargs: t.Any) -> Future:
        if not self.is_alive():
            raise RuntimeError("Cannot submit new function: worker is down")
        f: Future = Future()
...
def _stop_workers(lock: t.Optional[threading.Lock], workers, wait=True) -> None:
    if lock:
        lock.acquire()
    try:
        for w in workers:
...
class SimpleThreadPoolExecutor:
    ...
    def __init__(self, max_workers: int, *, name: str = None) -> None:
        ...
        self._is_down = False
        weakref.finalize(self, _stop_workers, self._shutdown_lock, self._workers, True)
    ...
    def submit(self, target: t.Callable, /, *args: t.Any, **kwargs: t.Any) -> Future:
        with self._shutdown_lock:
            if self._is_down:
                raise RuntimeError("Cannot submit new function after shutdown")
            f: Future = Future()
            self._queue.put(Task(f, target, args, kwargs))
            return f
    ...
    def shutdown(self, wait=True) -> None:
        with self._shutdown_lock:
            if self._is_down:
                return
            self._is_down = True
            _stop_workers(None, self._workers, wait=wait)
